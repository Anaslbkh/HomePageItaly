...
export default {
  ...
  computed: {
    ...
    prevDays() {
      ...
      const daysCount = this.mondayFirst
        ? (this.utils.getDay(dObj) > 0 ? this.utils.getDay(dObj) - 1 : 6)
        : this.utils.getDay(dObj)

      for (let i = 1; i <= daysCount; i++) {
        this.utils.setDate(dObj, this.utils.getDate(dObj) - 1)
        days.push(this.formatDateObj(dObj, { isInCurrentMonth: false }))
      }
