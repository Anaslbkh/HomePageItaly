<template>
  <div
    v-show="showYearView"
    ref="yearPicker"
    :class="[calendarClass, 'vdp-datepicker__calendar year-selector']"
    :style="calendarStyle"
    @mousedown.prevent
  >
    ...
    <span
      v-for="year in years"
      :key="year.timestamp"
      :ref="year.isSelected ? 'selected' : year.timestamp"
      class="cell year"
...
export default {
  ...
  methods: {
     ...
     */
    isDisabledYear(date) {
      // TODO: hardcoded false in order to not disable previous years in selector
      return false
