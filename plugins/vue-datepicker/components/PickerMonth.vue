<template>
  <div
    ...
    ref="monthPicker"
    :class="[calendarClass, 'vdp-datepicker__calendar month-selector']"
    :style="calendarStyle"
    @mousedown.prevent
  >
    ...
    <span
      ...
      :key="month.timestamp"
      :ref="(month.isSelected ? 'selected' : month.timestamp)"
      class="cell month"
...
export default {
  ...
  watch: {
    showMonthView() {
      setTimeout(() => {
        this.$refs.monthPicker.scrollTo({ top: this.$refs.selected[0].offsetTop })
      }, 0)
    }
  },
  methods: {
