<template>
  <section class="bg-blue-900 relative z-20 text-white -mt-6" v-if="!$fetchState.pending">
    <div class="container mx-auto py-12">
      <div class="flex justify-center mb-12">
        <ul class="flex">
          <li v-for="(link, social) in language.socials" :key="social">
            <a :href="link" target="_blank" rel="noopener nofollow" :aria-label="social"
               class="w-12 h-12 mx-3 flex items-center justify-center border border-white rounded hover:border-gray-700">
              <img
      ...
      <div class="flex justify-center leading-loose flex-col sm:flex-row">
        ...
        <a v-if="termsConditionsPageContent.url" :href="termsConditionsPageContent.url"
           class="mx-4 text-white hover:text-white">{{ termsConditionsPageContent.title }}</a>
        <a v-if="privacyPolicyPageContent.url" :href="privacyPolicyPageContent.url"
           class="mx-4 text-white hover:text-white">{{ privacyPolicyPageContent.title }}</a>
      </div>
    </div>
  </section>
</template>
