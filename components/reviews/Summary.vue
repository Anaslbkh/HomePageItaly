<template>
  ...
  >
    ...
    <meta
      itemprop="brand"
      :content="airportData.airport.devtitle.replace('-', ' ')"
    />
    <div
      itemprop="offers"
      itemscope=""
    ...
    >
      ...
      <meta
        itemprop="lowPrice"
        :content="(airportData.airport.from_price / 8).toFixed(2)"
      />
    </div>
    <div
      ...
      <p class="text-3xl mb-10 font-heading">
        {{
          $i18n("templates.customer-parking-reviews-title", {
            ...
            reviewsScoreAvg: new Intl.NumberFormat("it").format(
              meta.score.toFixed(1)
            ), // @TODO dynamic number format
          })
        }}
      </p>
      <p
        class="text-base"
        v-html="
          $i18n('templates.customer-parking-reviews-intro-schema', {
            airportName: airportData.airport.name,
